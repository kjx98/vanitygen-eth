...
vg_ocl_platform_getstr(cl_platform_id pid, cl_platform_info param)
{
	...
	cl_int ret;
	size_t size_ret;
	ret = clGetPlatformInfo(pid, param, sizeof(platform_str), platform_str, &size_ret);
	if (ret != CL_SUCCESS) {
...
vg_ocl_device_getplatform(cl_device_id did)
{
	...
	if (ret != CL_SUCCESS) {
		fprintf(stderr, "clGetDeviceInfo(CL_DEVICE_PLATFORM): %s", vg_ocl_strerror(ret));
	}
	return val;
}
...
vg_ocl_device_getstr(cl_device_id did, cl_device_info param)
{
	...
	if (ret != CL_SUCCESS) {
		snprintf(device_str, sizeof(device_str), "clGetDeviceInfo(%d): %s",
...
vg_ocl_device_getsizet(cl_device_id did, cl_device_info param)
{
	...
	if (ret != CL_SUCCESS) {
		fprintf(stderr, "clGetDeviceInfo(%d): %s", param, vg_ocl_strerror(ret));
	}
	return val;
}
...
enum {
	...
	VG_OCL_OPTIMIZATIONS        = (VG_OCL_DEEP_PREPROC_UNROLL |
				       ...
				       VG_OCL_DEEP_VLIW | VG_OCL_AMD_BFI_INT),
		    ...
		    const char *program, size_t size, unsigned char *hash_out)
{
	...
	EVP_DigestUpdate(mdctx, str, strlen(str) + 1);
	if (opts) EVP_DigestUpdate(mdctx, opts, strlen(opts) + 1);
	if (size) EVP_DigestUpdate(mdctx, program, size);
...
vg_ocl_amd_patch_inner(unsigned char *binary, size_t size)
{
	...
	if ((size < sizeof(*ehp)) ||
	    memcmp(ehp->e_ident, "\x7f" "ELF\1\1\1\x64", 8) || !ehp->e_shoff)
		return 0;
...
vg_ocl_amd_patch(vg_ocl_context_t *vocp, unsigned char *binary, size_t size)
{
	...
	ehp = (vg_elf32_header_t *) binary;
	if ((size < sizeof(*ehp)) ||
	    memcmp(ehp->e_ident, "\x7f" "ELF\1\1\1\0", 8) || !ehp->e_shoff)
		return 0;

	offset = 1;
	while (offset < (size - 8)) {
		ptr = (unsigned char *) memchr(binary + offset, 0x7f, size - offset);
		if (!ptr) return npatched;
		offset = ptr - binary;
		...
		npatched += nrun;
		if (vcp->vc_verbose > 1)
			fprintf(stderr, "AMD BFI_INT: patched %d instructions in kernel %d\n",
                    nrun, ninner);
		npatched++;
		offset += 1;
	}
		    ...
		    const char *filename, const char *opts)
{
	...
	if (vocp->voc_quirks & VG_OCL_NO_BINARIES) {
		kfp = NULL;
		if (vcp->vc_verbose > 1) fprintf(stderr, "Binary OpenCL programs disabled\n");
	} else {
	...
	if (!kfp) {
		...
		while (!feof(kfp)) {
			...
			if (szr == sz) {
				...
				if (!tbuf) {
					fprintf(stderr, "Could not expand CL kernel binary buffer\n");
					fclose(kfp);
					free(buf);
					return 0;
				}
		...
		fclose(kfp);
	rebuild:
		prog = clCreateProgramWithBinary(vocp->voc_oclctx, 1, &vocp->voc_ocldid,
						 &szr, (const unsigned char **) &buf, &sts, &ret);
	}
	free(buf);
	if (!prog) {
	...
	if (fromsource && !(vocp->voc_quirks & VG_OCL_NO_BINARIES)) {
		...
		if (!kfp) {
			fprintf(stderr, "WARNING: could not save CL kernel binary: %s\n",
				strerror(errno));
		} else {
			sz = fwrite(buf, 1, szr, kfp);
			fclose(kfp);
			if (sz != szr) {
				fprintf(stderr, "WARNING: short write on CL kernel "
...
vg_ocl_init(vg_context_t *vcp, vg_ocl_context_t *vocp, cl_device_id did, int safe_mode)
{
	...
	if (vcp->vc_verbose > 1) vg_ocl_dump_info(vocp);
	...
	vocp->voc_oclctx = clCreateContext(NULL, 1, &did, vg_ocl_context_callback,
					   NULL, &ret);
	if (!vocp->voc_oclctx) {
	...
	if (!vg_ocl_load_program(vcp, vocp, "calc_addrs.cl", optbuf)) return 0;
	return 1;
}
			...
			int arg, size_t size, int host)
{
	...
	for (i = 0; i < MAX_SLOT; i++) {
		...
		for (j = 0; vg_ocl_arg_map[arg][j] >= 0; j += 2) {
			...
			ret = clSetKernelArg(vocp->voc_oclkernel[i][knum], karg, sizeof(clbuf), &clbuf);
		   ...
		   void *buffer, size_t size)
{
	...
	ret = clEnqueueWriteBuffer(vocp->voc_oclcmdq, vocp->voc_args[slot][arg],
				   CL_TRUE, 0, size, buffer, 0, NULL, NULL);
...
static void *
vg_ocl_map_arg_buffer(vg_ocl_context_t *vocp, int slot, int arg, int rw)
{
	void *buf;
	cl_int ret;
...
static void
vg_ocl_unmap_arg_buffer(vg_ocl_context_t *vocp, int slot, int arg, void *buf)
{
	cl_int ret;
	cl_event ev;
...
int
vg_ocl_kernel_int_arg(vg_ocl_context_t *vocp, int slot, int arg, int value)
{
	cl_int ret;
	int i;
			 ...
			 int arg, void *value, size_t size)
{
	...
	for (i = 0; i < MAX_SLOT; i++) {
		...
		for (j = 0; vg_ocl_arg_map[arg][j] >= 0; j += 2) {
			knum = vg_ocl_arg_map[arg][j];
			karg = vg_ocl_arg_map[arg][j+1];
			ret = clSetKernelArg(vocp->voc_oclkernel[i][knum], karg, size, value);
			if (ret) {
				fprintf(stderr, "clSetKernelArg(%d,%d): ", knum, karg);
				vg_ocl_error(vocp, ret, NULL);
				return 0;
...
static int
vg_ocl_kernel_start(vg_ocl_context_t *vocp, int slot, int ncol, int nrow, int invsize)
{
	cl_int val, ret;
	cl_event ev;
...
vg_ocl_gethash_check(vg_ocl_context_t *vocp, int slot)
{
	...
	ocl_hashes_out = (unsigned char *)vg_ocl_map_arg_buffer(vocp, slot, 0, 0);

	if (!ocl_hashes_out) {
		fprintf(stderr, "ERROR: Could not map hash result buffer for slot %d\n", slot);
		return 2;
	}
...
vg_ocl_prefix_rekey(vg_ocl_context_t *vocp)
{
	...
	for (i = 0; i < vocp->voc_nslots; i++) {
		ocl_found_out = (uint32_t *)vg_ocl_map_arg_buffer(vocp, i, 0, 1);
		if (!ocl_found_out) {
			fprintf(stderr,
				"ERROR: Could not map result buffer for slot %d (rekey)\n", i);
			return -1;
		}
	...
	if (vocp->voc_pattern_rewrite) {
		...
		if (i > vocp->voc_pattern_alloc) {
			...
			if (!vg_ocl_kernel_arg_alloc(vocp, -1, 5, 40 * i, 0)) return -1;
			vocp->voc_pattern_alloc = i;
		}

		/* Write range records */
		ocl_targets_in = (unsigned char *) vg_ocl_map_arg_buffer(vocp, 0, 5, 1);
		if (!ocl_targets_in) {
...
vg_ocl_prefix_check(vg_ocl_context_t *vocp, int slot)
{
	...
	if (!ocl_found_out) {
		fprintf(stderr,
			"ERROR: Could not map result buffer for slot %d\n", slot);
		return 2;
	}
...
vg_ocl_prefix_init(vg_ocl_context_t *vocp)
{
	...
	if (!vg_ocl_create_kernel(vocp, 2, "hash_ec_point_search_prefix")) return 0;

	for (i = 0; i < vocp->voc_nslots; i++) {
		if (!vg_ocl_kernel_arg_alloc(vocp, i, 0, 28, 1)) return 0;
	}
...
vg_ocl_config_pattern(vg_ocl_context_t *vocp)
{
	...
	i = vg_context_addr_sort(vcp, NULL);
	if (i > 0) {
		if (vcp->vc_verbose > 1) fprintf(stderr, "Using OpenCL prefix matcher\n");
		/* Configure for prefix matching */
		return vg_ocl_prefix_init(vocp);
	}

	if (vcp->vc_verbose > 0) fprintf(stderr, "WARNING: Using CPU pattern matcher\n");
...
vg_ocl_verify_temporary(vg_ocl_context_t *vocp, int slot, int z_inverted)
{
	...
	z_heap = (unsigned char *)vg_ocl_map_arg_buffer(vocp, slot, 1, 0);
	point_tmp = (unsigned char *)vg_ocl_map_arg_buffer(vocp, slot, 2, 0);
	ocl_points_in = (unsigned char *)vg_ocl_map_arg_buffer(vocp, slot, 3, 0);
	ocl_strides_in = (unsigned char *)vg_ocl_map_arg_buffer(vocp, slot, 4, 0);
...
vg_opencl_loop(vg_exec_context_t *arg)
{
	...
	if (!pbatchinc || !poffset || !pseek) goto enomem;
	...
	if (!vg_ocl_config_pattern(vocp)) goto enomem;
	...
	for (i = 0; i < nslots; i++) {
		 ...
		 * - Column point array
		 */
		if (!vg_ocl_kernel_arg_alloc(vocp, i, 4, 64 * 2 * nrows, 1)) goto enomem;
	}
	...
	if (pthread_create(&vocp->voc_ocl_thread, NULL, vg_opencl_thread, vocp))
		goto enomem;
...
l_rekey:
	...
	if (vcp->vc_privkey_prefix_length > 0) {
		BIGNUM *pkbn = BN_dup(EC_KEY_get0_private_key(pkey));
        RAND_bytes((unsigned char *)pkbn->d, 32 - vcp->vc_privkey_prefix_length);
		EC_KEY_set_private_key(pkey, pkbn);
	...
	while (1) {
		...
		if ((npoints + round) < rekey_at) {
			if (npoints > 1) {
				...
				for (i = 0; i < nrows; i++) {
					EC_POINT_add(pgroup, pprow[i], pprow[i], poffset,
                                vxcp->vxc_bnctx);
				}
	...
	if (halt) {
		...
		if (vcp->vc_verbose > 1) printf("done!\n");
	}
	...
	if (ppbase) {
		for (i = 0; i < (nrows + ncols); i++)
			if (ppbase[i]) EC_POINT_free(ppbase[i]);
		free(ppbase);
	}
...
show_devices(cl_platform_id pid, cl_device_id *ids, int nd, int base)
{
	...
	for (i = 0; i < nd; i++) {
		...
		if (len >= sizeof(nbuf)) len = sizeof(nbuf) - 1;
		nbuf[len] = '\0';
		res = clGetDeviceInfo(ids[i], CL_DEVICE_VENDOR, sizeof(vbuf), vbuf, &len);
		if (res != CL_SUCCESS) continue;
		if (len >= sizeof(vbuf)) len = sizeof(vbuf) - 1;
		vbuf[len] = '\0';
		   ...
		   int worksize, int nthreads, int nrows, int ncols, int invsize)
{
	 ...
	 */
	if (!worksize) {
		if (vg_ocl_device_gettype(vocp->voc_ocldid) & CL_DEVICE_TYPE_GPU)
			worksize = 2048;
		else
			worksize = 256;
	}
	...
	if ((round % invsize) || !is_pow2(invsize) || (invsize < 2)) {
		...
		if (round % invsize)
			fprintf(stderr,
				"Modular inverse work size must evenly divide points\n");
		else
			fprintf(stderr, "Modular inverse work per task (%d) "
			       ...
			       int safemode, int verify)
{
	...
	*part2 = '\0';
		...
		*param = '\0';
		...
		if (!strcmp(part, "grid")) {
			...
			if (invsize & (invsize - 1)) {
				fprintf(stderr, "Modular inverse size %d must be "
					"a power of 2\n", invsize);
				invsize = 0;
				continue;
			}
	...
	return vg_ocl_context_new(vcp, platformidx, deviceidx, safemode,
				  verify, worksize, nthreads, nrows, ncols, invsize);
}
